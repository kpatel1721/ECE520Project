...
*****************************************************************************************************************************/
int ReadVec(FILE *fvec, PATTERN *p){
  char input[Mlin];
  int i; 
  int row = 0; 
  int col;
  while(fgets(input, sizeof(input), fvec) != NULL){
    int length = 0; 
    //for loop to find length of each row
    for(i = 0; i < Mlin; i++){
      if(input[i] == '\0'){break;}
      else{length++;}
    }
    ...
    for(i = 0; i < length; i++){
      ...
      else if(input[i] == '1'){p[row].matrix[i] = 1;}
      else if(input[i] == '0'){p[row].matrix[i] = 0;}
